...
const BUNDLE_EDITOR_HELP: Record<Locale, {
  notation: string; jsonRows: string; pair: string; criteria: string; component: string; relation: string; example: string;
}> = {
  en: { notation: 'Every text field supports inline and display math, chemistry, units, and inline code. Multiline prose fields also support fenced code blocks.', jsonRows: 'For multi-column fields, enter one JSON row per line.', pair: 'JSON per line: ["item", "detail"]', criteria: 'JSON per line: ["criterion", ["value 1", "value 2"]]', component: 'JSON per line: ["id", "label", "role", "parent id"]', relation: 'JSON per line: ["source id", "target id", "relationship"]', example: 'JSON per line: ["text", "translation", "note"]' },
  ja: { notation: 'すべてのテキスト欄で行内・別行数式、化学式、単位、行内コードを使えます。複数行の文章欄ではフェンス付きコードブロックも使えます。', jsonRows: '複数列の欄は1行に1つのJSON行で入力します。', pair: '1行ごとのJSON：["項目", "詳細"]', criteria: '1行ごとのJSON：["基準", ["値1", "値2"]]', component: '1行ごとのJSON：["ID", "名前", "役割", "親ID"]', relation: '1行ごとのJSON：["元ID", "先ID", "関係"]', example: '1行ごとのJSON：["本文", "翻訳", "注記"]' },
  'zh-CN': { notation: '所有文本字段都支持行内及独立公式、化学式、单位和行内代码；多行正文还支持围栏代码块。', jsonRows: '多列字段每行输入一条 JSON 记录。', pair: '每行 JSON：["项目", "详情"]', criteria: '每行 JSON：["标准", ["值一", "值二"]]', component: '每行 JSON：["ID", "名称", "角色", "父ID"]', relation: '每行 JSON：["源ID", "目标ID", "关系"]', example: '每行 JSON：["原文", "翻译", "注释"]' },
  es: { notation: 'Todos los campos admiten fórmulas en línea y separadas, química, unidades y código en línea. Los campos de prosa multilínea también admiten bloques de código cercados.', jsonRows: 'En campos con varias columnas, introduce una fila JSON por línea.', pair: 'JSON por línea: ["elemento", "detalle"]', criteria: 'JSON por línea: ["criterio", ["valor 1", "valor 2"]]', component: 'JSON por línea: ["id", "nombre", "función", "id superior"]', relation: 'JSON por línea: ["id origen", "id destino", "relación"]', example: 'JSON por línea: ["texto", "traducción", "nota"]' },
  ar: { notation: 'تدعم جميع الحقول النصية الرياضيات المضمنة والمنفصلة والكيمياء والوحدات والشفرة المضمنة. وتدعم حقول النص متعددة الأسطر كتل الشفرة المسيجة أيضاً.', jsonRows: 'في الحقول متعددة الأعمدة، أدخل صف JSON واحداً في كل سطر.', pair: 'JSON في كل سطر: ["العنصر", "التفاصيل"]', criteria: 'JSON في كل سطر: ["المعيار", ["القيمة 1", "القيمة 2"]]', component: 'JSON في كل سطر: ["المعرف", "الاسم", "الدور", "معرف الأصل"]', relation: 'JSON في كل سطر: ["المصدر", "الهدف", "العلاقة"]', example: 'JSON في كل سطر: ["النص", "الترجمة", "الملاحظة"]' },
  hi: { notation: 'हर टेक्स्ट फ़ील्ड में इनलाइन और अलग गणित, रसायन, इकाइयाँ और इनलाइन कोड काम करते हैं। बहु-पंक्ति विवरण फ़ील्ड में फ़ेन्स्ड कोड ब्लॉक भी काम करते हैं।', jsonRows: 'कई कॉलम वाले फ़ील्ड में हर पंक्ति पर एक JSON रो लिखें।', pair: 'हर पंक्ति JSON: ["बिंदु", "विवरण"]', criteria: 'हर पंक्ति JSON: ["मानदंड", ["मान 1", "मान 2"]]', component: 'हर पंक्ति JSON: ["id", "नाम", "भूमिका", "parent id"]', relation: 'हर पंक्ति JSON: ["स्रोत id", "लक्ष्य id", "संबंध"]', example: 'हर पंक्ति JSON: ["पाठ", "अनुवाद", "टिप्पणी"]' },
};
            ...
            <View style={styles.form}>
              ...
              {knowledgeType ? (
                <>
                  ...
                  <View accessibilityLabel={BUNDLE_EDITOR_HELP[locale].notation} style={styles.editorHelp}>
