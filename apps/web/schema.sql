...
-- Recall attempt rows retain only content-free lifecycle metadata. Private
-- prompts, answers, reconstructed order, provenance locators, and cues never
-- enter this table.
CREATE TABLE IF NOT EXISTS recall_attempts (
  ...
  CONSTRAINT recall_attempts_item_owner_fk
    ...
    REFERENCES user_knowledge_items(id, user_id)
    ON DELETE CASCADE,
  CONSTRAINT recall_attempts_item_version_check CHECK (item_version >= 1),
  CONSTRAINT recall_attempts_schedule_version_check CHECK (schedule_version >= 1),
  CONSTRAINT recall_attempts_milestone_check CHECK (milestone IN ('d1', 'd7')),
  ...
  CONSTRAINT recall_attempts_lifecycle_state_check
    CHECK (lifecycle_state IN ('prepared', 'confidence_selected', 'revealed', 'completed', 'invalidated')),
  CONSTRAINT recall_attempts_confidence_check
    CHECK (confidence IS NULL OR confidence IN ('low', 'medium', 'high')),
  CONSTRAINT recall_attempts_outcome_check
    CHECK (self_assessed_outcome IS NULL OR self_assessed_outcome IN ('remembered', 'partial', 'missed')),
  CONSTRAINT recall_attempts_duration_check
    CHECK (response_duration_bucket IS NULL OR response_duration_bucket IN ('under_30s', '30_to_89s', '90_to_179s', '3_to_5m', 'over_5m')),
  CONSTRAINT recall_attempts_invalidation_reason_check
    CHECK (invalidation_reason IS NULL OR invalidation_reason IN ('stale_context', 'item_removed')),
  ...
  CONSTRAINT recall_attempts_lifecycle_shape_check
    CHECK (COALESCE(
      ...
      OR (
        ...
        AND resulting_due_at IS NULL
        AND (
          (confidence IS NULL AND confidence_selected_at IS NULL AND revealed_at IS NULL)
          OR (confidence IS NOT NULL AND confidence_selected_at IS NOT NULL)
        )
      ),
      FALSE
    ))
