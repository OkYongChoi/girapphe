...
export default async function RecallPage() {
  ...
  const [{ t }, enrollmentEnabled] = await Promise.all([
    getServerI18n(),
    Promise.resolve(isRecallRuntimeEnrollmentEnabledForUser(user.id)),
  ]);
  const overview = await getManualRecallOverviewForUser(user.id, { enrollmentEnabled });
