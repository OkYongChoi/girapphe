...
function ResolutionButtons({ hasTarget, blocked }: { hasTarget: boolean; blocked: boolean }) {
  ...
  return (
    <div className="flex min-w-0 flex-wrap gap-2">
      {hasTarget ? (
        <>
          <button name="resolution_action" value="merge" disabled={!hydrated || pending || blocked} onClick={(event) => { if (!window.confirm(t('resolution.mergeConfirm'))) event.preventDefault(); }} className="min-h-11 rounded-xl bg-violet-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-violet-700 disabled:opacity-50">
            {t('resolution.merge')}
          </button>
          <button name="resolution_action" value="update" disabled={!hydrated || pending || blocked} onClick={(event) => { if (!window.confirm(t('resolution.updateConfirm'))) event.preventDefault(); }} className="min-h-11 rounded-xl border border-amber-400 bg-amber-50 px-4 py-2.5 text-sm font-bold text-amber-900 hover:bg-amber-100 disabled:opacity-50">
            {t('resolution.update')}
          </button>
        </>
      ) : (
        <button name="resolution_action" value="create" disabled={pending || blocked} className="min-h-11 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-blue-700 disabled:opacity-50">
...
function BundlePreview({ value, legacyExplanation = false }: { value: Seed; legacyExplanation?: boolean }) {
  return value.knowledge_type && value.central_question && value.structured_content ? (
    <div className="mt-3 min-w-0 max-w-full rounded-xl border border-blue-100 bg-blue-50/50 p-3">
  ...
  ) : value.content ? <p className="mt-3 min-w-0 max-w-full line-clamp-5 whitespace-pre-wrap text-sm leading-relaxed text-slate-600"><KnowledgeText text={value.content} legacyDollarMath={legacyExplanation} /></p> : null;
}
...
export default function DraftResolutionPanel({
  ...
  return (
    <div className="grid w-full min-w-0 grid-cols-[minmax(0,1fr)] gap-6">
      <section className="min-w-0 max-w-full rounded-2xl border border-amber-200 bg-amber-50 p-5">
      ...
      <section aria-labelledby="duplicate-heading" className="min-w-0 max-w-full">
        ...
        {duplicateSuggestions.length === 0 ? (
          ...
          <ol className="mt-3 grid min-w-0 grid-cols-[minmax(0,1fr)] gap-3 md:grid-cols-2">
            {duplicateSuggestions.map((suggestion) => (
              <li key={suggestion.id} className={`min-w-0 rounded-2xl border bg-white p-4 ${target?.id === suggestion.id ? 'border-violet-400 ring-2 ring-violet-100' : 'border-slate-200'}`}>
      ...
      {target && existing ? (
        <section aria-labelledby="comparison-heading" className="min-w-0 max-w-full">
          ...
          <div className="mt-3 grid min-w-0 grid-cols-[minmax(0,1fr)] gap-4 lg:grid-cols-2">
            <article className="min-w-0 rounded-2xl border border-amber-200 bg-white p-5">
            ...
            <article className="min-w-0 rounded-2xl border border-violet-200 bg-white p-5">
      ...
      <section aria-labelledby="final-heading" className="min-w-0 max-w-full rounded-2xl border border-blue-200 bg-white p-5 shadow-sm md:p-6">
        <div className="flex min-w-0 flex-wrap items-end justify-between gap-3">
          <div className="min-w-0">
          ...
          {existing ? (
            <div className="flex min-w-0 max-w-full flex-wrap rounded-xl border border-slate-200 bg-slate-50 p-1 text-xs font-bold">
        ...
        <form key={`${target?.id ?? 'new'}:${seedChoice}`} action={resolve} className="mt-5 grid min-w-0 max-w-full grid-cols-[minmax(0,1fr)] gap-4">
          ...
          <div className="grid min-w-0 grid-cols-[minmax(0,1fr)] gap-4 md:grid-cols-2">
            <label className="grid min-w-0 grid-cols-[minmax(0,1fr)] gap-1 text-xs font-bold text-slate-700">{t('notes.titleLabel')}<input name="title" required maxLength={120} defaultValue={seed.title} className="min-h-11 w-full min-w-0 max-w-full rounded-lg border px-3 text-sm font-normal focus:outline-none focus:ring-2 focus:ring-blue-400" /></label>
            <label className="grid min-w-0 grid-cols-[minmax(0,1fr)] gap-1 text-xs font-bold text-slate-700">{t('notes.newTopic')}<input name="topic" required maxLength={120} defaultValue={seed.topic} className="min-h-11 w-full min-w-0 max-w-full rounded-lg border px-3 text-sm font-normal focus:outline-none focus:ring-2 focus:ring-blue-400" /></label>
          </div>
          ...
          <label className="grid min-w-0 grid-cols-[minmax(0,1fr)] gap-1 text-xs font-bold text-slate-700">{t('knowledge.summaryLabel')}<textarea name="summary" maxLength={500} defaultValue={seed.summary} className="min-h-20 w-full min-w-0 max-w-full rounded-lg border p-3 text-sm font-normal focus:outline-none focus:ring-2 focus:ring-blue-400" /></label>
          <label className="grid min-w-0 grid-cols-[minmax(0,1fr)] gap-1 text-xs font-bold text-slate-700">{t('notes.contentLabel')}<textarea name="content" maxLength={6000} defaultValue={seed.content} className="min-h-32 w-full min-w-0 max-w-full rounded-lg border p-3 text-sm font-normal focus:outline-none focus:ring-2 focus:ring-blue-400" /></label>
          <label className="grid min-w-0 grid-cols-[minmax(0,1fr)] gap-1 text-xs font-bold text-slate-700">{t('knowledge.tagsLabel')}<input name="tags" maxLength={599} defaultValue={seed.tags.join(', ')} className="min-h-11 w-full min-w-0 max-w-full rounded-lg border px-3 text-sm font-normal focus:outline-none focus:ring-2 focus:ring-blue-400" /></label>
          ...
          <details open={draft.relations.some((relation) => (
            ...
            <div className="mt-3 grid min-w-0 grid-cols-[minmax(0,1fr)] gap-3 sm:grid-cols-2">
              <label className="grid min-w-0 grid-cols-[minmax(0,1fr)] gap-1 text-xs font-bold text-slate-700">{t('bundle.field.occurredAt')}<input type="datetime-local" name="observed_at" defaultValue={lifecycleDefaults.observed_at} className="min-h-11 w-full min-w-0 max-w-full rounded-lg border bg-white px-3 text-sm font-normal" /></label>
              <label className="grid min-w-0 grid-cols-[minmax(0,1fr)] gap-1 text-xs font-bold text-slate-700">{t('topic.lifecycle.reviewAt')}<input type="datetime-local" name="review_at" defaultValue={lifecycleDefaults.review_at} className="min-h-11 w-full min-w-0 max-w-full rounded-lg border bg-white px-3 text-sm font-normal" /></label>
              <label className="grid min-w-0 grid-cols-[minmax(0,1fr)] gap-1 text-xs font-bold text-slate-700">{t('notes.from')}<input type="datetime-local" name="valid_from" defaultValue={lifecycleDefaults.valid_from} className="min-h-11 w-full min-w-0 max-w-full rounded-lg border bg-white px-3 text-sm font-normal" /></label>
              <label className="grid min-w-0 grid-cols-[minmax(0,1fr)] gap-1 text-xs font-bold text-slate-700">{t('notes.to')}<input type="datetime-local" name="valid_to" defaultValue={lifecycleDefaults.valid_to} className="min-h-11 w-full min-w-0 max-w-full rounded-lg border bg-white px-3 text-sm font-normal" /></label>
            </div>
              ...
              <fieldset className="mt-4 min-w-0 max-w-full">
                ...
                <div className="mt-2 grid min-w-0 grid-cols-[minmax(0,1fr)] gap-2">
                  {draft.relations.map((relation, index) => {
                    ...
                    return (
                      <label key={`${relation.targetKind}:${relation.targetId}:${relation.type}:${index}`} className={`flex min-w-0 items-start gap-2 rounded-lg border bg-white p-3 text-xs ${causal ? 'border-amber-300 text-amber-950' : 'border-slate-200 text-slate-700'}`}>
                        <input
                          type="checkbox"
              ...
              <fieldset className="mt-4 min-w-0 max-w-full">
                ...
                <div className="mt-2 grid min-w-0 grid-cols-[minmax(0,1fr)] gap-2">
                  {draft.proposed_evidence.map((evidence, index) => {
                    ...
                    return (
                    <label key={`${evidence.selectorType}:${index}`} className="flex min-w-0 items-start gap-2 rounded-lg border border-slate-200 bg-white p-2 text-xs text-slate-700">
                      <input
                        type="checkbox"
                        ...
                        className="mt-0.5 h-4 w-4 shrink-0 rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                      />
                      <span className="min-w-0 [overflow-wrap:anywhere]"><strong>#{index + 1} · {t(location.key, location.values)}</strong> · {evidence.polarity} · {evidence.quality} · {t(`topic.graph.origin.${evidence.relationOrigin}` as MessageKey)}</span>
                    </label>
                    );
                  })}
                </div>
        ...
        <form action={ignore} className="mt-4 min-w-0 max-w-full border-t border-slate-100 pt-4">
          <input type="hidden" name="batch_id" value={batchId} />
          <input type="hidden" name="draft_id" value={draft.id} />
          <input type="hidden" name="draft_version" value={draft.version} />
          <IgnoreButton />
