...
- `expression` is a version-one knowledge bundle with a BCP 47 language tag,
  meanings, translations, pronunciation, register, nuance, usage contexts,
  examples, contrasts, and common mistakes.
...
- Private relations accept `causes`, `contributes_to`, `enables`, and `inhibits`.
  Extracted or model-proposed causal relations require at least one reviewed
  evidence selector; manual user links may be explicit without imported evidence.
...
- Shared/web parsers reject invalid language tags, chronology ranges, and causal
  evidence indexes.
