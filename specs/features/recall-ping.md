...
| `R1-08` | `messages.test.ts` checks all six catalogs; `recall-runtime.test.ts` checks logical-direction classes and 44px controls. `authenticated-recall.spec.ts` is the desktop/mobile rendered Preview gate and asserts measured 44px control bounds plus Arabic RTL direction/containment while recording zero browser errors, action timing/size metrics, and three screenshots per device. |
